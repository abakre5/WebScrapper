...
def parse(browser, contests, noOfRows):
    parentDirPath = CollectorUtil.createCollectorImageDir("../Data/OpenContestSubmissionImages", log)
    try:
        urlNo = 1
        for url in contests:
            index = 0
            ...
            if not ((result[0].lower() == 'finished' and result[1] == 1) or (result[0].lower() == 'locked')):
                while True:
                    ...
                    CollectorUtil.getCollectorImage(contestID, data, parentDirPath, True, log)
...
if __name__ == "__main__":
    log.basicConfig(filename='../scratch/logs/OpenContestSubmissionInfoCollector' + str(date.today()) + '.log',
